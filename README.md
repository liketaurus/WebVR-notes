![Cardboard Icon](VRicon.png) 
#WebVR Resources
----
## Slide decks
- [Hacking reality: browser based VR (2014)*](http://www.slideshare.net/auradeluxe/hacking-reality-browserbased-vr-fall-2014)
- [Bringing Virtual Reality to the Web: VR, WebGL and CSS – Together At Last!](http://www.slideshare.net/fitc_slideshare/web-vr-fitc)
- [Web VR Slide decks from Toni Parisi](http://www.slideshare.net/auradeluxe?utm_campaign=profiletracking&utm_medium=sssite&utm_source=ssslideview) 
## Tutorials
- [4 Steps to start with WebVR in 10 minutes](http://www.roadtovr.com/4-steps-to-start-experimenting-with-webvr-in-10-minutes/)
- [How to build VR on the web](https://www.sitepoint.com/how-to-build-vr-on-the-web-today/) 
- [Official guides for A-Frame*](https://aframe.io/docs/0.3.0/guides/)
- [Awesome WebVR resources list](https://github.com/wizztjh/awesome-WebVR)
## Technologies and libraries
- [Mozilla VR*](https://mozvr.com/)
- [WebVR Api documentation](https://developer.mozilla.org/en-US/docs/Web/API/WebVR_API)
- [A-Frame: A web framework for building virtual reality experiences](https://aframe.io/)
- [GLAM: GL And Markup - declarative language for 3D web content](http://tparisi.github.io/glam/) 
- [WebVR Polyfill: use WebVR today, without requiring a special browser build](https://github.com/googlevr/webvr-polyfill)
## Samples, Demos, Boilerplates and Tools
- [Chrome Experiments for Virtual Reality*](http://vr.chromeexperiments.com/)
- [WebVR Samples](https://webvr.info/samples/)
- [A-Frame Starter Kit: Create your first "Hello World" WebVR scene*](https://github.com/aframevr/aframe-boilerplate)
- [Vizor: a platform for creating and sharing WebVR content](https://github.com/vizorvr/vizor) 
	- [Online Vizor editor*](http://vizor.io/) 



